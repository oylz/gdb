          ...
          int num, struct ui_file *stream)
{
  ...
  if (num != -1){
      ...
      print_frame_nameless_args(frame, start, num - args_printed, first, stream);
  }
...
print_frame_info(struct frame_info *frame, int print_level,
  ...
  if (get_frame_type (frame) == DUMMY_FRAME
      ...
      struct cleanup *uiout_cleanup = make_cleanup_ui_out_tuple_begin_end (uiout, "frame");
  ...
  if ((disassemble_next_line == AUTO_BOOLEAN_AUTO
    ...
    do_gdb_disassembly(get_frame_arch(frame), 
        1,
        get_frame_pc(frame), 
